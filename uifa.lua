...
local script_url = "https://raw.githubusercontent.com/yuunusarslann/uifa/master/uifa.luac"
...
--Level 1 Commands
  ...
  {
    name = "msk",
    reason = "Spawnkilling is not allowed! Leave the spawn area.",
    mute = 0,
    ban = 0,
    mban = 0,
    disarm = 0,
  ...
  {
    name = "mct",
    reason = "Cross-teaming is not allowed!",
    mute = 0,
    ban = 0,
    mban = 0,
    disarm = 0,
  ...
  {
    name = "mvip",
    reason = "For donation and information about VIP status, refer to uifserver.net/vip",
    mute = 0,
    ban = 0,
    mban = 0,
    disarm = 0,
  ...
  {
    name = "madm",
    reason = "For donation and information about Admin status, refer to uifserver.net/admin",
    mute = 0,
    ban = 0,
    mban = 0,
    disarm = 0,
...
{00FFFF}/msk{FFFFFF} - Sends an /amsg for spawnkilling.
{00FFFF}/mct{FFFFFF} - Sends an /amsg for cross-teaming.
{00FFFF}/mafk{FFFFFF} - Sends an /amsg to check if the player is active or not. (eg: "Move if you're active")
{00FFFF}/mvip{FFFFFF} - Sends an /amsg about VIP information (eg: When someone asks how to donate for VIP, use this).
{00FFFF}/madm{FFFFFF} - Sends an /amsg about Admin information (eg: When someone asks how to donate for Admin, use this).
]]
...
{00FFFF}/jh - {FFFFFF}Mode bans the player for jump hacks.
...
{00FFFF}/rbst - {FFFFFF}Shortcut for /rbstats. (/rbs [ID])
...
{00FFFF}/sts [ID] - {FFFFFF}Shortcut for /stats.
{00FFFF}/pst [ID] - {FFFFFF}Shortcut for /pnetstats.
...
local news = [[{00FF00}UIF Assistant v0.9a Update {FFFFFF}- 13.04.2021

{00FFFF}General
{00FF00}- {FFFFFF}Reworked on source code to make it more optizimied and simpler. (Thanks to Hammy)
{00FF00}- {FFFFFF}Implemented an auto-update system. (Refer to script's topic for more information)
{00FF00}- {FFFFFF}Fixed incorrect game mode map order for /mrl command.

{00FFFF}Commands
{00FF00}- {FFFFFF}Level 1: New commands - /msk, /mct, /mafk, /mvip, /madm: (/help - LV1 FMI) (Trotsky's Suggestion)
{00FF00}- {FFFFFF}Level 2: New command - /jh: Mode bans the player for jump hacks.
{00FF00}- {FFFFFF}Level 3: Command Change - /rbst: /rbs has been converted to /rbst for consistency purposes. (Hammy's Suggestion)
{00FF00}- {FFFFFF}MISC: New command - /sts: Shortcut for /stats. (Transporter974's Suggestion)
{00FF00}- {FFFFFF}MISC: New command - /pst: Shortcut for /pnetstats. (Transporter974's Suggestion)
...
function main()
	...
	sampRegisterChatCommand("ver", functionUAvers)			
	...
	downloadUrlToFile(update_url, update_path, function(id, status)
		if status == dlstatus.STATUS_ENDDOWNLOADDATA then
			...
			if tonumber(updateIni.info.vers) > script_vers then
				sampAddChatMessage("[UIFA] {FFFFFF}New version of UIFA detected! {00FF00}", 0x00FFFF)
				update_state = true
			end
  ...
  if not sampRegisterChatCommand(t["name"], function(args)
    if #args ~= 0 then
      ...
      if id and id >= 0 and id <= 500 then
        ...
        if t["amsg"] ~= 0 then
          sampSendChat(string.format("/amsg %d %s", id, t["reason"]), 0x00FFFF)
        end
      else
...
if not sampRegisterChatCommand("pst", function(args)
    if #args > 0 then
        local id = math.floor(tonumber(args))
...
if not sampRegisterChatCommand("rbst", function(args)
    if #args > 0 then
        local id = math.floor(tonumber(args))
		...
		while sampIsDialogActive() do
			...
			if result then
				if button == 1 then
					...
					elseif list == 1 then
						sampAddChatMessage("{00FF00}[UIFA]: Successfully reloaded Derby room "..args.." to 2020", 0x00FFFF)				
					...
					elseif list == 11 then
						sampAddChatMessage("{00FF00}[UIFA]: Successfully reloaded Derby room "..args.." to Dead City", 0x00FFFF)				
					...
					elseif list == 18 then
						sampAddChatMessage("{00FF00}[UIFA]: Successfully reloaded Derby room "..args.." to Monster Tube", 0x00FFFF)				
					...
					elseif list == 23 then
						sampAddChatMessage("{00FF00}[UIFA]: Successfully reloaded Derby room "..args.." to Poseidon", 0x00FFFF)				
					...
					elseif list == 31 then
						sampAddChatMessage("{00FF00}[UIFA]: Successfully reloaded Derby room "..args.." to Space Ship", 0x00FFFF)				
					...
					elseif list == 36 then
						sampAddChatMessage("{00FF00}[UIFA]: Successfully reloaded Derby room "..args.." to Star Wars", 0x00FFFF)				
    	    		...
    	    		elseif list == 41 then
    	    			sampAddChatMessage("{00FF00}[UIFA]: Successfully reloaded Derby room "..args.." to Volcanic Islands", 0x00FFFF)				
    	    	    	sampSendChat(string.format('/rlderby '..args..' 40'))	    
					end
				else
					wait(100)
				end
